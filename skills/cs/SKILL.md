...
| BUG / 问题 / 异常 / 报错 / "这里不对" / "修复" / "有 bug" | `cs-diagnose`（自动判断快速通道或完整诊断） |
...
| 开发者指南 / 用户指南 | `cs-guide` |
| 库 API 参考 | `cs-libdoc` |
| 用户在 feature / issue 流程中间问"下一步" | 路由到对应入口（`cs-feat` / `cs-diagnose`），让该入口判断当前阶段 |
...
2. 6 实体 + 3 流程的速读图
3. 问"你现在最想从哪儿开始？"，给三个引子：
   - "我有个新功能想做" → cs-feat
   - "代码里有个 bug" → cs-diagnose
