...
**测试质量核查**（对照 implement 的 TDD 推进记录）：
- [ ] 所有测试都测公开接口（不测私有方法）
- [ ] 测试描述 WHAT 不描述 HOW（测试名是"用户可以..."而非"函数调用..."）
- [ ] 没有 mock 内部协作者
- [ ] 测试会在重构后继续通过（验证方式：试着重命名内部函数，测试仍通过）
...
- [ ] 第 3 节场景核对全部勾选，前端已浏览器验证，测试质量核查通过
- [ ] 第 4 节术语一致性无遗漏
...
- **测试质量核查走过场**——没真的检查测试是否测行为（只看测试通过不看测试质量）
- **测试 mock 了内部协作者但没发现**——这种测试在重构时会失败，验收时要识别出来
