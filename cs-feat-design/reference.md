...
steps:
  - action: "{行为切片}：{用户可感知的行为变化描述}"
    behavior: "{这个切片测试什么行为（WHAT）}"
    exit_signal: "{测试通过 + 行为可验证}"
    status: pending
...
- 粒度是行为切片（behavior slices），**不写 file:line / 函数级**——具体落点是 implement 的事
- 切片原则"从 design 第 3 节关键场景清单提取"：
  - 每个切片 = 一个用户可感知的行为变化
  - 每个切片对应 1-2 个测试
  - 切片按依赖顺序排列（如"基础结账"必须在"结账后清空购物车"之前）
  - 每个切片在 implement 阶段走完整 TDD 闭环（RED → GREEN → REFACTOR）
- 4-8 步；每步必须有可独立验证的退出信号（"测试通过 + 行为可验证"）
...
按行为切片（behavior slices），4-8 行讲清切片顺序 + 每步退出信号。详细清单落 `{slug}-checklist.yaml` 的 `steps`。**只写行为切片，不下沉到 file:line**。

从第 3 节"关键场景清单"提取行为切片，每个切片 = 一个可验证的行为变化，implement 阶段走 TDD 闭环。

示例（购物车结账功能）：

```
1. 基础结账：用户可以用有效购物车结账
   退出信号：测试通过，返回订单 ID 和确认状态
2. 空购物车校验：空购物车无法结账
   退出信号：测试通过，抛出预期错误
3. 购物车清空：结账后购物车自动清空
   退出信号：测试通过，购物车为空
4. 支付失败处理：支付失败时不扣款不清空购物车
   退出信号：测试通过，购物车状态未变
```

**切片粒度判断**：
- ✅ 恰当：每个切片对应 1-2 个测试，用户可感知
- ❌ 太大：一个切片包含 3+ 个独立行为（如"完整结账流程 + 邮件 + 库存 + 发票"）→ 拆分
- ❌ 太小：一个切片不值得独立测试（如"生成订单 ID"、"设置状态"）→ 合并

**切片顺序**：按依赖关系排列，基础行为在前，扩展行为在后。
...
> 7. 第 2.4 推进策略：行为切片是否从第 3 节场景清单正确提取？切片粒度是否合适（每个对应 1-2 个测试）？顺序是否按依赖关系排列？
> 8. 第 3 节验收场景：覆盖正常 + 边界 + 错误路径了吗？
>
> 有修改意见直接说，确认后进入实现阶段。
